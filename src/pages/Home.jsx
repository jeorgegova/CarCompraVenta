...
const Home = () => {
  ...
  // Obtener conteo total con filtros desde Supabase
  const fetchTotalCount = useCallback(async () => {
    try {
      const { count, error } = await runQuery((s) => {
        let q = s.from('vehicles').select('*', { count: 'exact', head: true }).eq('status', 'approved');
        ...
        if (filters.selectedYears.length > 0) {
          q = q.in('year', filters.selectedYears);
        }
        
        // Precio range
        if (filters.selectedPriceRange !== null) {
          if (filters.selectedPriceRange.min > 0) {
            q = q.gte('price', filters.selectedPriceRange.min);
          }
          if (filters.selectedPriceRange.max !== Infinity) {
            q = q.lte('price', filters.selectedPriceRange.max);
          }
        ...
        // Kilometraje range
        if (filters.selectedMileageRange !== null) {
          if (filters.selectedMileageRange.min > 0) {
            q = q.gte('mileage', filters.selectedMileageRange.min);
          }
          if (filters.selectedMileageRange.max !== Infinity) {
            q = q.lte('mileage', filters.selectedMileageRange.max);
          }
        } else {
          if (filters.mileage_min) q = q.gte('mileage', filters.mileage_min);
          if (filters.mileage_max) q = q.lte('mileage', filters.mileage_max);
        }
  ...
  const fetchFilterOptions = useCallback(async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        // Contar vehículos por rango (sin incluir "Todos")
            ...
            <main className="flex-1">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
                <h2 className="text-xl font-semibold text-gray-900">
                  Vehículos en venta ({totalCount > 0 ? totalCount : 'Cargando...'})
                </h2>
